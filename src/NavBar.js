...
import { isCartVisible, setIsMobileMenuOpen } from "./CreateSlice";

function NavBar() {
	const dispatch = useDispatch();
	const cart = useSelector((state) => state.commerce.cart);
	const check = cart.length ? 1 : 0;

	return (
		<nav className="p-6 text-gray-600 bg-white">
			<div className="flex items-center justify-between">
				<div className="flex items-center space-x-20">
					<div className="flex items-center space-x-5">
						<button
							onClick={() => dispatch(setIsMobileMenuOpen())}
							className="md:hidden"
						>
				...
				<div className="flex items-center space-x-6 lg:space-x-11">
					...
					<img
						...
						className="rounded h-7 w-7 md:h-8 md:w-8 lg:h-10 lg:w-10"
						alt="Avatar"
					></img>
				</div>
			</div>
			<hr className="hidden my-10 border-t border-gray-300 md:block" />
		</nav>
	);
}
